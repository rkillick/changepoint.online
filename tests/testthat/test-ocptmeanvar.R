...
data <- list(singmeandata, mulmeandata, nochangedata, constantdata, negativedata)
...
for(p in penalties.error){
  if(p=="CROPS"){
    expect_that(ocpt.meanvar.initialise(singmeanvardata,penalty = p),throws_error("Use cpt.meanvar from changepoint as CROPS is not available with changepoint.online"))
  }else{
    expect_that(ocpt.meanvar.initialise(singmeanvardata,penalty = p),throws_error("Asymptotic penalty values must be > 0 and <= 1"))
  }
}
