...
online.single.mean.norm<-function(data,penalty="MBIC",pen.value=0,class=TRUE,param.estimates=TRUE,minseglen){
  if(is.null(dim(data))==TRUE){
    # single dataset
    n=length(data)
  }
  ...
  if(is.null(dim(data))==TRUE){ # single dataset
		...
		tmp=online.single.mean.norm.calc(data,extrainf=TRUE,minseglen)
		if(penalty=="MBIC"){
		  tmp[,3]=tmp[,3]+log(tmp[,1])+log(n-tmp[,1]+1)
		}
		ans=online.decision(tmp[,1],tmp[,2],tmp[,3],penalty,n,diffparam=1,pen.value)
...
online.single.var.norm.calc <- function(data,mu,extrainf=TRUE,minseglen){
  n=length(data)
...
online.single.var.norm<-function(data,penalty="MBIC",pen.value=0,know.mean=FALSE,mu=NA,class=TRUE,param.estimates=TRUE,minseglen){
  if(is.null(dim(data))==TRUE){
    # single dataset
    n=length(data)
    mu=mu[1]
  ...
  if(is.null(dim(data))==TRUE){
    ...
    tmp=online.single.var.norm.calc(coredata(data),mu,extrainf=TRUE,minseglen)
		if(penalty=="MBIC"){
		  tmp[3]=tmp[3]+log(tmp[1])+log(n-tmp[1]+1)
		}
		ans=online.decision(tmp[1],tmp[2],tmp[3],penalty,n,diffparam=1,pen.value)
    ...
    for(i in 1:rep){
      ...
      tmp[i,]=online.single.var.norm.calc(data[i,],mu[i],extrainf=TRUE,minseglen)
    }
...
online.single.meanvar.norm.calc <-
function(data,extrainf=TRUE,minseglen){
  singledim=function(data,extrainf=TRUE,minseglen){
    n=length(data)
    y=c(0,cumsum(data))
...
online.single.meanvar.norm<-function(data,penalty="MBIC",pen.value=0,class=TRUE,param.estimates=TRUE,minseglen){
  if(is.null(dim(data))==TRUE){
    # single dataset
    n=length(data)
  }
  ...
  if(is.null(dim(data))==TRUE){
		tmp=online.single.meanvar.norm.calc(coredata(data),extrainf=TRUE,minseglen)
		if(penalty=="MBIC"){
		  tmp[3]=tmp[3]+log(tmp[1])+log(n-tmp[1]+1)
		}
      ...
      return(online.class_input(data, cpttype="mean and variance", method="AMOC", test.stat="Normal", penalty=penalty, pen.value=ans$pen, minseglen=minseglen, param.estimates=param.estimates, out=c(0,ans$cpt)))
		...
		tmp=online.single.meanvar.norm.calc(data,extrainf=TRUE,minseglen)
		if(penalty=="MBIC"){
		  tmp[,3]=tmp[,3]+log(tmp[,1])+log(n-tmp[,1]+1)
		}
		ans=online.decision(tmp[,1],tmp[,2],tmp[,3],penalty,n,diffparam=2,pen.value)
